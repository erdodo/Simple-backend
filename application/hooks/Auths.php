...
class Auths extends CI_Controller
{
	...
	{
		$this->benchmark->mark('request_time');
		...
		if ($params['standart'] == 'v1') {
			...
			$this->benchmark->mark('auths_show_basi');
			$auths = (array) $this->auths_model->show('auths', $auths_config);
			if (empty($auths))
				res_error(["message" => "auths_not_found", "auths_type" => $fun, "table_name" => $params['table'], "status" => "error"], 401);

			$this->benchmark->mark('auths_detail_basi');
			$this->input->auths = $this->detail("tr", 'auths', $auths['id']); //db_show('auths',$auths['id']);// 

			$this->benchmark->mark('logs_basi');
				...
				if (intval($data->$clm_name) > 0) {
					...
					foreach ($relation_columns as $rc_key => $rc_value) { //istenilen bağlı kolonlar döngüye alınır
						...
						$data->$clm_name[$rc_value] =
							$this->lang_translate(!empty($relation_columns_record[$rc_value]) ? $relation_columns_record[$rc_value] : "", $rc_value);
					}
				} else {
					...
					if (is_array($degerler) || is_object($degerler)) {
						...
						foreach ($relation_columns as $rc_key => $rc_value) {
							...
							$data->$clm_name[$rc_value] = $this->lang_translate(!empty($gecici3[$rc_value]) ? $gecici3[$rc_value] : "", $rc_value);
						}
					}
				}
			}
	...
	public function lang_translate($data, $column)
	{
			...
			foreach ($field_list as $value) {
				...
				if (!empty($column_data)) {
					foreach ($column_data as $k => $v) {
						...
						$fields[$value][$k] = $this->lang_translate($v, $k);
					}
