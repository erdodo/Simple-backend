...
function db_list($table_name)
{
	...
	//Default filtreler
	$ci->benchmark->mark('getDBFilters');
	...
	$ci->benchmark->mark('table_list');
	$datas = $ci->base_model->list($table_name, $config);

	$ci->benchmark->mark('table_count');
	$datas = (array) $datas;
	$all_record_count = $ci->base_model->count($table_name, $config);
	$page_count = intval(ceil($all_record_count / ($body_limit ?? 50)));

	//Table bilgileri
	$ci->benchmark->mark('table_show');
	$table_info = $ci->db->select('display')->from('lists')->where('name', $table_name)->get()->row(); //db_show('lists','name:'.$table_name)['data'];
	$table_info->display = lang_translate($table_info->display, 'display');

	$ci->benchmark->mark('get_columns');
	//Tabloya ait kolonlar
	$fields = get_columns($table_name);
	//$enums = $ci->getEnums($fields);

	if (empty($datas))
		return (["fields" => $fields, "message" => "data_not_found", "status" => "success"]);

	//Yetkisine göre kolon gizleme
	$ci->benchmark->mark('getHideFields');
	...
	$ci->benchmark->mark('field_list_show');
	...
	$ci->benchmark->mark('request_time_end');
	return $response;
...
function db_show($table_name, $filter)
{
	...
	if (empty($data))
		res_error(["message" => "data_not_found", "status" => "error"]);
...
function getDBFilters($table_name, $type)
{
	...
	if (empty($auths))
		res_error(["message" => "auths_not_found", "auths_type" => $type, "table_name" => $table_name, "status" => "error"], 401);
	/*-------------------------------*/
	...
	foreach ($where as $k => $val) {
		...
		if (empty($val['codes']))
			continue;
		if (
			empty(key((array) eval($val['codes']))) ||
	...
	foreach ($fields as $clm_name => $clm) {
		...
		foreach ($datas as $key => $v) {
			...
			if ($clm['lang_support'] == 1) {
				...
				$datas[$key]->$clm_name = local_lang_translate($value[$clm_name], $clm);
			}
			if (!empty($clm['relation_table'])) {
				//NOTE - Eğer kolonun bağlı oldupu bir tablo var ise bu fonksiyon çalışır.
				if (intval($datas[$key]->$clm_name) > 0) { // Eğerki kayıtta id tutuluyorsa bu fonksiyon çalışır
					...
					// Gelen id verisinin bağlı olduğu tablodaki karşılığı istenir
					$relation_display_query = $ci->db->select($relation_display)->where("id", $gecici_id)->from($clm['relation_table'])->get();
					$relation_display_record = $relation_display_query ? (array) $relation_display_query->row() : NULL;
					foreach ($relation_display as $rc_value) { //istenilen bağlı kolonlar döngüye alınır
						//kayıt bir objeye dönüştürülerek id ve diğer kolonlar yazılır
						if (empty($datas[$key]->$clm_name["show"]))
							$datas[$key]->$clm_name["show"] = "";
						if (!empty($relation_display_record[$rc_value])) {
							$datas[$key]->$clm_name["show"] .= lang_translate($relation_display_record[$rc_value], $rc_value);
						} else {
							$datas[$key]->$clm_name["show"] .= $rc_value;
						}
					}

					$relation_columns_query = $ci->db->select($relation_columns)->where("id", $gecici_id)->from($clm['relation_table'])->get();
					$relation_columns_record = $relation_columns_query ? (array) $relation_columns_query->row() : NULL;
					foreach ($relation_columns as $rc_key => $rc_value) { //istenilen bağlı kolonlar döngüye alınır
						...
						$datas[$key]->$clm_name[$rc_value] =
							lang_translate(!empty($relation_columns_record[$rc_value]) ? $relation_columns_record[$rc_value] : "", $rc_value);
					}
				} else {
					...
					if (is_array($degerler) || is_object($degerler)) {
						...
						foreach ($degerler as $r_value) {
							...
							foreach ($relation_display as $rc_key => $rc_value) { //istenilen bağlı kolonlar döngüye alınır
								...
								//kayıt bir objeye dönüştürülerek id ve diğer kolonlar yazılır
								if (empty($datas[$key]->$clm_name[$r_value]["show"]))
									$datas[$key]->$clm_name[$r_value]["show"] = "";
								if (!empty($relation_columns_record[$rc_value])) {
									$datas[$key]->$clm_name[$r_value]["show"] .= lang_translate($relation_columns_record[$rc_value], $rc_value);
								} else {
						...
						foreach ($relation_display as $rc_key => $rc_value) { //istenilen bağlı kolonlar döngüye alınır
							...
							if (empty($datas[$key]->$clm_name["show"]))
								$datas[$key]->$clm_name["show"] = "";
							if (!empty($relation_columns_record[$rc_value])) {
								$datas[$key]->$clm_name["show"] .= lang_translate($relation_columns_record[$rc_value], $rc_value);
							} else {
								$datas[$key]->$clm_name["show"] .= $rc_value;
							}
						...
						foreach ($relation_columns as $rc_key => $rc_value) {
							...
							$datas[$key]->$clm_name[$rc_value] = lang_translate(!empty($gecici3[$rc_value]) ? $gecici3[$rc_value] : "", $rc_value);
						}
					}
				}
			}
	...
	if (!empty($data)) {
		foreach ($fields as $clm_name => $clm) {
			...
			if ($clm['lang_support'] == 1) {
				//NOTE - Eğer kolonda dil desteği var ise seçili dile uygun veri döndürme fonksiyonu
				// Seçili dilde veri yoksa eğer varsayılan olara türkçe döner
				$data->$clm_name = lang_translate($data->$clm_name, $clm_name);
				...
				if (intval($data->$clm_name) > 0) {
					...
					foreach ($relation_display as $rc_key => $rc_value) { //istenilen bağlı kolonlar döngüye alınır
						...
						$relation_columns_record =
							(array) $ci->base_model->show($clm['relation_table'], $relation_columns_record_config); //geçiçi olarak kaydettiğimiz id ile gerçek veriye ulaşılır
						//kayıt bir objeye dönüştürülerek id ve diğer kolonlar yazılır
						if (empty($data->$clm_name["show"]))
							$data->$clm_name["show"] = "";
						if (!empty($relation_columns_record[$rc_value])) {
							$data->$clm_name["show"] .= lang_translate($relation_columns_record[$rc_value], $rc_value);
						} else {
					...
					if (is_array($degerler) || is_object($degerler)) {
						...
						foreach ($degerler as $r_value) {
							...
							foreach ($relation_display as $rc_key => $rc_value) { //istenilen bağlı kolonlar döngüye alınır
								...
								$relation_columns_record =
									(array) $ci->base_model->show($clm['relation_table'], $relation_columns_record_config); //geçiçi olarak kaydettiğimiz id ile gerçek veriye ulaşılır
								//kayıt bir objeye dönüştürülerek id ve diğer kolonlar yazılır
								if (empty($data->$clm_name[$r_value]["show"]))
									$data->$clm_name[$r_value]["show"] = "";
								if (!empty($relation_columns_record[$rc_value])) {
									$data->$clm_name[$r_value]["show"] .= lang_translate($relation_columns_record[$rc_value], $rc_value);
								} else {
						...
						foreach ($relation_columns as $rc_key => $rc_value) {
							...
							$data->$clm_name[$rc_value] = lang_translate(!empty($gecici3[$rc_value]) ? $gecici3[$rc_value] : "", $rc_value);
						}
					}
				}
			}
...
function db_update($table_name, $filter)
{
	...
	if ($response['status'] == "error")
		res_error($response);
	if ($table_name == 'lists')
		$updated_data = edit_table($updated_data, $filtered_data);
...
function lang_translate($data, $column)
{
	$ci = get_instance();
	$ci->load->model('base_model');
	$ci->user = (array) $ci->input->user;
...
function local_lang_translate($data, $column)
{
	$ci = get_instance();
	$ci->load->model('base_model');
	$ci->user = (array) $ci->input->user;
	...
	if (!empty($get))
		$params = $get;
	/*-------------------------------------*/
	if ($column['lang_support'] == '1') {
		$selected_lang = 'tr';
		if (!empty($ci->user['language_id'])) {
			$selected_lang = $ci->db->select('name')->where('id', $ci->user['language_id'])->get('language')->row()->name;
		}
...
function get_columns($table_name)
{
	...
	$ci->benchmark->mark('get_columns_list_field');
	$fields = [];
	foreach ($field_list as $value) {
		$column_data = $ci->db->from('fields')->where('name', $value)->get()->row();

		if (!empty($column_data)) {
			foreach ($column_data as $k => $v) {
				$fields[$value][$k] = local_lang_translate($v, fields_field_list()[$k]);
			}

		}
	}
	$ci->benchmark->mark('get_columns_field_setting');
	return $fields;
}
	...
	if (!empty($field->relation_table)) {
		...
		foreach ($ad_table_data as $key => $value) {
			...
			foreach ($relation_display as $v) { //istenilen bağlı kolonlar döngüye alınır
				...
				if (empty($new_val["show"]))
					$new_val["show"] = "";

				if (!empty($val[$v])) {
					$new_val["show"] .= lang_translate($val[$v], $v);
				} else {
					$new_val["show"] .= $v;
				}
			...
			foreach ($relation_columns as $v) {
				$new_val[$v] = lang_translate($val[$v], $v);
			}
