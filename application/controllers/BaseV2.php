...
class BaseV2 extends CI_Controller
{
    ...
    public function file_upload()
    {
        ...
        if ($data)
            res_success(["data" => $data, "status" => "success"]);
        else
            res_error(["message" => "error", "status" => "error"]);
    }
    ...
    public function front_cache()
    {
        ...
        foreach ($auths as $value) {
            if (empty($response['auths'][$value->table_name]))
                $response['auths'][$value->table_name] = [];
        ...
        foreach ($front_langs as $value) {
            $response['front_langs'][$value->name] = lang_translate($value->display, 'display');
        ...
        foreach ($language as $value) {
            $response['language'][$value->name] = lang_translate($value->display, 'display');
        ...
        foreach ($table_group as $value) {
            ...
            foreach ($table_group_tables as $value2) {
                ...
                $new_table_group_tables[$value2] = $table_display ? lang_translate($table_display->row()->display, 'display') : NULL;
            }
            $response['table_group'][$value->name] = [
                "display" => lang_translate($value->display, 'display'),
                "icon" => $value->icon,
